            ...
            case 0x1c: {
                ...
                case 0x6: { /* CSRRSI */
                    if (insn.rd) {
                        switch (insn.csr) {
                        case fflags:
                            sprintf(buf, "x%d = (uint64_t)(state->fcsr & 0x1f);\n", insn.rd);
                            break;
                        case frm:
                            sprintf(buf, "x%d = (uint64_t)((state->fcsr & 0x7) >> 5);\n", insn.rd);
                            break;
                        case fcsr:
                            sprintf(buf, "x%d = (uint64_t)(state->fcsr & 0xff);\n", insn.rd);
                            break;
                    ...
                    if (insn.rs1 != 0) {
                        ...
                        switch (insn.csr) {
                        case fflags:
                            sprintf(buf, "state->fcsr |= (%s%d & 0x1f);\n", x, insn.rs1);
                            break;
                        case frm:
                            sprintf(buf, "state->fcsr |= ((%s%d & 0x7) << 5);\n", x, insn.rs1);
                            break;
                        case fcsr:
                            sprintf(buf, "state->fcsr |= (%s%d & 0xff);\n", x, insn.rs1);
                            break;
                        default: fatal("unsupported csr");
                        }
                        body = str_append(body, buf);
